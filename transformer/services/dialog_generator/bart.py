...
class BartDialogGenerator(ServiceInterface):
    def load_model(self, model_dir):
        ...
        self.tokenizer = load_tokenizer(path=model_dir, model_type="bart")
        ...
        print("loading model...")
        self.model = Bart(vocab_size=self.vocab_size)
    ...
    def infer_next_utterance_greedy(self, utterances: List[str], speaker_ids: List[str], conditions: List[str],
        ...
        input_ids, type_ids, decoder_input_ids = self.encode(utterances=utterances, speaker_ids=speaker_ids, conditions=conditions, max_retry=max_retry)
    ...
    def infer_next_utterance_beam_search(self, utterances: List[str], speaker_ids: List[str], conditions: List[str],
        ...
        input_ids, token_type_ids, decoder_input_ids = self.encode(utterances=utterances, speaker_ids=speaker_ids, conditions=conditions, max_retry=max_retry)
        candidates = self.model.generate(input_ids,
                                         decoder_input_ids=decoder_input_ids,
                                         min_length=min_length,
                                         max_length=self.timesteps,
    ...
    def encode(self, utterances: List[str], speaker_ids: List[int], conditions: List[str], max_retry: int):
        ...
        def _encode_row(utterances: List[str], speaker_ids: List[int], conditions: List[str]):
            input_ids = []
            token_type_ids = []
            ...
            _head_token_type_id = token_type_ids[0] if len(token_type_ids) > 0 else self.user_speaker_id
            _tail_token_type_id = token_type_ids[-1] if len(token_type_ids) > 0 else self.user_speaker_id
            token_type_ids = [_head_token_type_id] + token_type_ids + [_tail_token_type_id]
        ...
        decoder_input_ids = [self.tokenizer.eos_token_id, self.tokenizer.bos_token_id]

        input_ids = convert_to_tensor([input_ids], device=self.device)
        token_type_ids = convert_to_tensor([token_type_ids], device=self.device)
        decoder_input_ids = convert_to_tensor([decoder_input_ids], device=self.device).to(torch.long)
        return input_ids, token_type_ids, decoder_input_ids